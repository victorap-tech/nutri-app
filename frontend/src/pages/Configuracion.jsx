...
export default function Configuracion() {
  ...
  const [firma, setFirma]     = useState(null);  // base64 string
  ...
  const handleFirma = (e) => {
    ...
    reader.onload = (ev) => setFirma(ev.target.result); // data:image/png;base64,...
    ...
    <div className="container">
      ...
      <div className="card" style={{ maxWidth: 580 }}>
            ...
            {firma ? (
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <img
                  src={firma}
                  alt="Firma"
                  style={{ maxHeight: 80, maxWidth: 240, objectFit: "contain", background: "white", padding: 4, borderRadius: 6, border: "1px solid var(--border-light)" }}
                />
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <button className="btn-secondary" style={{ fontSize: "0.8rem", padding: "6px 12px" }} onClick={() => fileRef.current.click()}>
                    Cambiar imagen
                  </button>
                  <button className="btn-danger" style={{ fontSize: "0.8rem", padding: "6px 12px" }} onClick={borrarFirma}>
                    Quitar firma
                  </button>
                </div>
              </div>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: 12 }}>
                  Subí una imagen de tu firma (PNG con fondo transparente recomendado)
                </p>
                <button className="btn-secondary" style={{ fontSize: "0.85rem" }} onClick={() => fileRef.current.click()}>
                  Seleccionar imagen
                </button>
              </div>
            )}
        ...
        {(form.prof_nombre || firma) && (
          <div style={{
            marginTop: 8, marginBottom: 20, padding: "16px 20px",
            background: "var(--surface-2)", borderRadius: "var(--radius)",
            border: "1px solid var(--border-light)",
          }}>
            <div style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text-muted)", marginBottom: 12 }}>
              Preview firma en PDF
            </div>
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, textAlign: "center", maxWidth: 220, margin: "0 auto" }}>
              {firma && (
                <img src={firma} alt="firma" style={{ maxHeight: 60, maxWidth: 180, objectFit: "contain", marginBottom: 8, display: "block", margin: "0 auto 8px" }} />
              )}
